Pro preprocessing,cube,header,writecube,log=log,catalogue=outputcatalogue,noise=noise,name=name,directory=dir,dir_format=dirformat,mask=mask
...
;-
  ...
  IF tmp[0] NE -1 and not KEYWORD_SET(mask) then begin
     IF size(log,/TYPE) EQ 7 then begin
        openu,66,log,/APPEND
  ...
  ;  tmp[0]=-1   ;This problem appears to be fixed now. But back again
    IF tmp[0] NE -1 then begin
       IF size(log,/TYPE) EQ 7 then begin
          openu,66,log,/APPEND
