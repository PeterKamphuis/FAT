...
;  calculate the smoothed profiles
  ...
  ; We want to make sure that our profile is declining
  rising = 0
  FOR j=n_elements(PAsmooth)-2,0,-1 DO BEGIN
      IF PAsmooth[j+1] GT PAsmooth[j] THEN BEGIN
        rising = 1
        PAsmooth[j] = PAsmooth[j+1]
      ENDIF
  ENDFOR
  IF size(log,/TYPE) EQ 7 AND rising EQ 1 THEN BEGIN
    openu,66,log,/APPEND
    printf,66,linenumber()+'REGULARISATION_SDIS: We found a rising profile and have adjusted it.'
    close,66
  ENDIF
