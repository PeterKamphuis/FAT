Pro parameterreguv87,PAin,SBRin,RADIIin,error=errorin,fixedrings=fixedringsin,REVERSE=rev,NOWEIGHT=noweight,Difference=DDivin,cutoff=cutoffin,arctan=arctanin,debug=debug,nocentral=nocentral,order=order,max_deviation=maxdevin,max_par=PAmaxin,min_par=PAminin,accuracy=accuracy,extending=extending
        ...
        for order=endorder,2,-1 do begin
           ...
           For x=0,9 do begin
              ...
              WHILE mccount LT its do begin
                 ...
                 newPAcoeff=POLY_FIT(fitRADII,fitPA,order,CHISQ=tmp,measure_errors=fiterrors,sigma=sigma,STATUS=fitstat,/DOUBLE)
                 IF keyword_set(debug) then begin 
...
;we dislike declining rotation curves so let's check the last 1/4.
        ...
        IF not keyword_set(rev) then begin
           ...
           newPAcoeff=POLY_FIT(fitRADII,fitPA,0.,CHISQ=tmp,measure_errors= straighterrors,sigma=sigma,STATUS=fitstat,/DOUBLE)
