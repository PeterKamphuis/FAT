     ...
     IF keyword_set(maxdev) then begin
        ...
        for j=n_elements(PA)-2,fix(n_elements(PA)/2.),-1 do begin
           ...
           IF ABS(deltaslope) GT maxdev then begin
              ...
              ;      IF j EQ n_elements(PA)-2 then begin
              ;         IF SBR[n_elements(PA)-1] GT cutoff[n_elements(PA)-1] then begin
...
;not want to get rid of them completely
  ...
  ;                     errors[j]=ABS((PA[j]-PA[j-1])/2.)
        ...
        ENDIF
        IF keyword_set(debug) then begin
           print,'errors after fixedringsds'
           print,errors
        ENDIF
     ENDIF
  ...
  ;IF resetlasterror then errors[n_elements(errors)-1]=errors[n_elements(errors)-2] 
  IF keyword_set(debug) then begin
...
shifterrors=fiterrors
  case arctan of
     0:begin 
        ...
        endorder=ceil((n_elements(fitPA)-3.)/2.)
        ...
        for order=endorder,2,-1 do begin
           ...
           fitfailed=0.
           IF keyword_set(rev) then begin
              newPAcoeff=FAT_FIT(fitRADII,fitPA,order,RCHI_SQR=tmp,errors=fiterrors,STATUS=fitstat,/Monte_Carlo,mc_iters=150000.,$
                                 mc_errors=shifterrors,maximum_deviation=maxdev,accuracy=accuracy,fixedrings=fixedrings,$
                                 mc_y_min=pamin,mc_y_max=pamax,mc_ddiv=ddiv,nocentral=nocentral,mc_maxfails=100.,/debug,log=log)
           ENDIF ELSE BEGIN
               newPAcoeff=FAT_FIT(fitRADII,fitPA,order,RCHI_SQR=tmp,errors=fiterrors,STATUS=fitstat,/Monte_Carlo,mc_iters=150000.,$
                                 mc_errors=shifterrors,maximum_deviation=maxdev,accuracy=accuracy,fixedrings=fixedrings,$
                                 mc_y_min=pamin,mc_y_max=pamax,nocentral=nocentral,mc_maxfails=100.,/debug,log=log)
           ENDELSE
        ...
        IF not keyword_set(rev) then begin
           ...
           newPAcoeff=FAT_FIT(fitRADII,fitPA,0.,RCHI_SQR=redChi,errors= straighterrors,STATUS=fitstat,log=log)
...
;and then the normal averaging
     ...
     ELSE:begin
        print,'REVISED_REGULARISATION: this should never happen'
        stop
     ENDELSE
  ENDCASE
  ...
  IF fixedrings GT 0. and order NE 0 then begin
     ...
     ENDIF ELSE BEGIN
        ...
        if locmax GT fixedrings then newPA[0:fixedrings]=TOTAL(newPA[0:fixedrings])/n_elements(newPA[0:fixedrings])
     ENDELSE
  ENDIF
 
  IF n_elements(errorin) NE 0. AND n_elements(mcerrors) GT 0. AND order GT 0. AND FINITE(order) then begin
     errorin=dblarr(n_elements(PA)) 
     tmperr=dblarr(n_elements(PA)) 
     tmperr2=dblarr(n_elements(PA)) 
     IF keyword_set(rev) then begin
        if keyword_set(nocentral) then begin
            tmperr[1:n_elements(tmperr)-1]=REVERSE(mcerrors[*,order-2])
        ENDIF ELSE BEGIN
           tmperr[*]=REVERSE(mcerrors[*,order-2])
        ENDELSE
     ...
     ENDIF ELSE BEGIN
        ...
        if keyword_set(nocentral) then begin
           tmperr[1:n_elements(tmperr)-1]=mcerrors[*,order-2]
        ENDIF ELSE BEGIN
           tmperr[*]=mcerrors[*,order-2]
        ENDELSE
  ...
  IF tmp[0] NE -1  then begin
     IF keyword_set(rev) then begin
        IF tmp[0] NE 0 then  errorin[tmp]=PAin[tmp]-pamin else  begin
           IF n_elements(tmp) GT 1 then errorin[tmp[1:n_elements(tmp)-1]]=PAin[tmp[1:n_elements(tmp)-1]]-pamin
        ENDELSE
     endif else begin
        errorin[tmp]=PAin[tmp]-pamin
     endelse
  ENDIF
  IF SBRin[n_elements(PAin)-1] LT cutoff[n_elements(PAin)-1] then errorin[n_elements(PAin)-1]=MAX(errorin)
  If errorin[n_elements(errorin)-1] LT errorin[n_elements(errorin)-2] then errorin[n_elements(errorin)-1]=errorin[n_elements(errorin)-2]
