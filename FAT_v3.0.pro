...
noconfig:
  ...
  for i=startgalaxy,endgalaxy do begin 
     ...
     tmp=WHERE(dummy LT -5.*catnoise[i])
     ...
     IF tmp[0] NE -1 then begin
        ...
        IF size(log,/TYPE) EQ 7 then begin
           openu,66,log,/APPEND
           printf,66,linenumber()+' Your cube had values below 5 sigma. If you do not have a central absorption source there is something seriously wrong with the cube'
           
           close,66
        ENDIF ELSE BEGIN
           print,linenumber()+' Your cube had values below 5 sigma. If you do not have a central absorption source there is something seriously wrong with the cube'
 ...
 ;    ringcorrection=0.
     ...
     printf,1,format='(A25,A25,A20,A20,A20,A20,A20,A20,A20,A20,A20,A20)',string(RAhr+'+/-'+strtrim(strcompress(string(RAdiff,format='(F6.1)')),2)),$
            ...
            string(strtrim(strcompress(string(Totflux[0],format='(E8.1)')),2)),$
            string(strtrim(strcompress(string(DHI,format='(F8.1)')),2)),$
            string(strtrim(strcompress(string(catDistance[i])),2)),$
            string(strtrim(strcompress(string(HIMass[0],format='(E8.1)')),2)),$
     ...
     tirificfirst[tmppos]='CFLUX= '+string((totflux[0])/7.5E5)
     tmppos=where('CFLUX_2' EQ tirificfirstvars)
     tirificfirst[tmppos]='CFLUX_2= '+string((totflux[0])/7.5E5)
...
;******************************************This is the end of the first fit******************************
     ...
     IF norings LE 4 then begin     
        IF norings GE 3 then finishafter=1.1 else begin
           openu,1,outputcatalogue,/APPEND
